...
router.post('/contact', async (req, res) => {
  ...
  if (discordWebhook && discordWebhook !== '') {
    try {
      const embed = {
        embeds: [{
          ...
          fields: [
            ...
            {
              name: '📋 Subject',
              value: subject || 'General Inquiry',
              inline: false
            }
          ],
          footer: {
            text: 'MOD Delivery - Midgets Out for Delivery 🐾'
          },
      ...
      res.status(500).json({ success: false, message: 'Failed to send message to Discord' });
    }
    return;
  }

  // Fallback to email if no Discord webhook
  const transporter = createTransporter();
  if (!transporter) {
